...
def save_metrics(metrics_list: List[Dict], out_dir: str = OUT_DIR) -> None:
    ...
    df_rows = [{**{k: m.get(k) for k in ["algorithm", "variant", "found", "time_s", "peak_memory_kb", "expansions", "status"]},
                "explored_count": len(m["explored"]) if m.get("explored") else None,
                "visited_count": len(m["visited"]) if m.get("visited") else None,
                "path_length": len(m["path"]) if m.get("path") else None} for m in metrics_list]
...
def print_metrics_block(title: str, res: Dict, time_s: float, peak_kb: float) -> None:
    visited = len(res["visited"]) if res.get("visited") else "N/A"
    print(f"\n=== {title} ===\nStatus        : {res.get('status')}\nFound         : {res.get('found')}\nPath length   : {len(res['path']) if res.get('path') else 'N/A'}\nExplored nodes: {len(res.get('explored', []))}\nVisited nodes : {visited}\nExpansions    : {res.get('expansions')}\nTime (s)      : {time_s:.6f}\nPeak memory KB: {peak_kb:.2f}")
...
def main():
    ...
    tracemalloc.stop()
    tree_time, tree_peak_kb = t1 - t0, peak / 1024.0
    graph_meas = measure_graph(dfs_graph_iterative, inflated, start, goal, "URDL")
    graph_res, graph_time, graph_peak_kb = graph_meas["result"], graph_meas["time_s"], graph_meas["peak_memory_kb"]

    # 6) prepare metrics
    metrics = [{"algorithm": "DFS", "variant": "tree-search", "found": tree_res.get("found"), "path": tree_res.get("path"),
                "explored": tree_res.get("explored"), "visited": None, "expansions": tree_res.get("expansions"),
                "status": tree_res.get("status"), "time_s": tree_time, "peak_memory_kb": tree_peak_kb},
               {"algorithm": "DFS", "variant": "graph-search", "found": graph_res.get("found"), "path": graph_res.get("path"),
                "explored": graph_res.get("explored"), "visited": list(graph_res["visited"]) if graph_res.get("visited") else None,
                "expansions": graph_res.get("expansions"), "status": graph_res.get("status"),
                "time_s": graph_time, "peak_memory_kb": graph_peak_kb}]
    ...
    # 8) save metrics and launch animator
    save_metrics(metrics)
    tree_explored, tree_path = tree_res.get("explored") or [], tree_res.get("path") or []
    graph_explored, graph_path = graph_res.get("explored") or [], graph_res.get("path") or []
